...
A second admissible hump is possible even when the short rate starts above the long-maturity level. For example, take
\begin{equation*}
    x_{t_0,t_0}=3.50\%,
    \qquad x_\infty=2.50\%,
    \qquad a^2=0.25,
...
By contrast, a curve that is already falling at the short end and then later develops a conventional interior hump generally requires more structure than the bare two-loading specification. Such behaviour is naturally interpreted as a policy or institutional distortion rather than as an undisturbed rolling-maturity equilibrium effect.

\begin{takeaway}
The scale parameters \(a^2\) and \(r\) set the maturity clocks. The loading split between \(A\) and \(B\) determines whether the observed curve is monotone, humped, or locally dipped.
\end{takeaway}
\end{proofblock}
...
The following stylised example shows that an ordinary upward-sloping conventional yield curve can coexist with a humped conventional-rate instantaneous volatility profile.
