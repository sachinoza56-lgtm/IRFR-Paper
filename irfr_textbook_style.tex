...
The same two-scale mechanism appears in the local variance-rate, but it is helpful to avoid introducing a separate shorthand for the two loadings. Keeping the coefficients in full form, the IRFR instantaneous variance-rate is
\[
...
\left[
\left(
 x_{t_0,t_0}-\frac{r}{1+r}x_\infty
\right)
e^{-a^2(T+t-2t_0)}
+
...
For fixed calendar time \(t\), write \(\tau=T-t\). Then the same expression is
\[
...
\left(
 x_{t_0,t_0}-\frac{r}{1+r}x_\infty
\right)
e^{-2a^2(t-t_0)}e^{-a^2\tau}
+
\frac{r}{1+r}x_\infty e^{-ra^2\tau}
\right]^2.
\]
This is a two-loading maturity function, but the loading coefficients have been left in their structural form. The coefficient on \(e^{-a^2\tau}\) is
\[
\left(
 x_{t_0,t_0}-\frac{r}{1+r}x_\infty
\right)e^{-2a^2(t-t_0)},
\]
and the coefficient on \(e^{-ra^2\tau}\) is
\[
\frac{r}{1+r}x_\infty.
\]
...
For fixed \(t\), differentiate the full-form variance-rate directly:
\[
\frac{\partial}{\partial t}\operatorname{Var}(x_{t,t+\tau})
=
2a^2
...
\right]^2.
\]
An interior critical point can occur either when the bracketed loading itself is zero, giving a zero of variance, or when the derivative of the bracketed loading is zero. The nonzero turning-point condition is therefore
\[
\left(
 x_{t_0,t_0}-\frac{r}{1+r}x_\infty
\right)
e^{-2a^2(t-t_0)}e^{-a^2\tau}
+
\frac{r^2}{1+r}x_\infty e^{-ra^2\tau}
=0.
\]
Equivalently,
\[
...
=
-
\frac{\dfrac{r^2}{1+r}x_\infty}
{\left(x_{t_0,t_0}-\dfrac{r}{1+r}x_\infty\right)e^{-2a^2(t-t_0)}}.
\]
Thus an interior nonzero turning point exists when the right-hand side is positive and the resulting maturity
\[
\tau^*
=
-\frac{1}{(1-r)a^2}
...
At such a point, the second derivative can be checked directly by differentiating the full-form bracketed loading once more. In the relevant parameter region, when the two full-form exponential components have opposite signs and different decay rates, the magnitude of the bracketed maturity loading can be largest away from the short end, producing a volatility hump.

The covariance-rate kernel remains one-factor. The covariance rate between maturities \(S_1\) and \(S_2\) is the product of the same full-form bracketed loading evaluated at \(S_1\) and \(S_2\), multiplied by \(2a^2\). Hence the kernel is still rank one.

\begin{takeaway}
The hump comes from the deterministic maturity profile inside the full-form bracketed loading, not from a second stochastic factor. The model is one-factor in randomness but two-loading in maturity.
\end{takeaway}
\end{proofblock}
...
A notational caution is important. The quantity \(x_{t_0,t_0}\) is not a structural parameter of the model. It is the short-end boundary value observed at the reconstruction date \(t_0\). Once that boundary value is fixed by the observed curve, it imposes the level restriction on the admissible maturity profile. In the full-form two-loading representation used below, the coefficient on \(e^{-a^2\tau}\) is \(x_{t_0,t_0}-r x_\infty/(1+r)\), while the coefficient on \(e^{-ra^2\tau}\) is \(-x_\infty/(1+r)\). These coefficients automatically sum to \(x_{t_0,t_0}-x_\infty\), so the notation does not introduce \(x_{t_0,t_0}\) as a freely calibrated parameter alongside \(a^2\), \(r\), or \(x_\infty\).

\begin{proofblock}{conditions for humped yield curves}
A convenient way to see the admissible yield-curve shapes is to write the initial rolling forward curve using the full structural loading coefficients:
\begin{equation*}
    x(t_0,t_0+\tau)
    =x_\infty
    +\left(x_{t_0,t_0}-\frac{r}{1+r}x_\infty\right)e^{-a^2\tau}
    -\frac{x_\infty}{1+r}e^{-ra^2\tau}.
\end{equation*}
This form makes the boundary role of \(x_{t_0,t_0}\) explicit. Setting \(\tau=0\) gives
\begin{equation*}
    x(t_0,t_0)=x_{t_0,t_0},
\end{equation*}
so \(x_{t_0,t_0}\) is the observed short-end boundary value, not an additional structural parameter. Equivalently, the two loading coefficients are fixed in full form by
\begin{equation*}
...
The associated conventional continuously compounded yield is therefore
\begin{equation*}
    y(t_0,t_0+\tau)
    =x_\infty
    +\left(x_{t_0,t_0}-\frac{r}{1+r}x_\infty\right)
      \frac{1-e^{-a^2\tau}}{a^2\tau}
    -\frac{x_\infty}{1+r}
      \frac{1-e^{-ra^2\tau}}{ra^2\tau}.
\end{equation*}
...
\begin{align*}
    ...
    &=-\frac{a^2}{2}\left[
      ...
      \right] \\
    &=-\frac{a^2}{2}\left(
        x_{t_0,t_0}-\frac{2r}{1+r}x_\infty
      \right).
\end{align*}
Thus the conventional yield curve initially slopes upward if
\begin{equation*}
    x_{t_0,t_0}-\frac{2r}{1+r}x_\infty<0,
\end{equation*}
...
\begin{align*}
    ...
    &=
    \frac{1}{a^2\tau}\left[
        x_{t_0,t_0}-\frac{r^2+1}{r(1+r)}x_\infty
    \right],
    \qquad \tau\to\infty .
\end{align*}
The sign of the bracketed full-form coefficient determines whether the conventional yield approaches the long-maturity level from above or below.
...
together with
\begin{equation*}
    x_{t_0,t_0}-\frac{r^2+1}{r(1+r)}x_\infty>0.
\end{equation*}
...
\begin{equation*}
    ...
    \qquad r=4.
\end{equation*}
Then the two full-form loading coefficients are
\begin{equation*}
    x_{t_0,t_0}-\frac{r}{1+r}x_\infty
    =3.50\%-\frac{4}{5}3.00\%=1.10\%,
\end{equation*}
 and
\begin{equation*}
    -\frac{x_\infty}{1+r}
    =-\frac{3.00\%}{5}=-0.60\%.
\end{equation*}
The boundary condition is automatically satisfied, since \(1.10\%-0.60\%=0.50\%=x_{t_0,t_0}-x_\infty\). The curve can therefore rise initially and then turn down toward \(x_\infty\) without treating \(x_{t_0,t_0}\) as a freely chosen structural parameter.

This humped-yield example should be distinguished from the upward-sloping, non-inverted numerical illustration used later, where \(x_\infty=4.00\%\), \(x_{t_0,t_0}=0.50\%\), \(a=0.70\), \(a^2=0.49\), and \(r=0.30\). That later parameter set is used to show that an ordinary upward-sloping conventional yield curve can coexist with a humped conventional-rate instantaneous volatility profile; it is not meant to be the same example as the interior-hump yield-curve condition in this proof block.

The same two-loading structure can generate positive or negative single-turning-point shapes, depending on the relative signs and magnitudes of the full-form maturity loadings. More complex multi-turning-point curves should not be viewed as a feature of the bare structural benchmark; they would naturally point to policy intervention, market segmentation, or another exogenous distortion.

\begin{takeaway}
The scale parameters \(a^2\) and \(r\) set the maturity clocks. The observed short-end value \(x_{t_0,t_0}\) fixes the boundary level at the reconstruction date; it is data, not an additional structural parameter. Conditional on that boundary, the full-form loading coefficients
\(x_{t_0,t_0}-r x_\infty/(1+r)\) and \(-x_\infty/(1+r)\) determine whether the reconstructed curve is monotone, humped, or locally dipped.
\end{takeaway}
\end{proofblock}
